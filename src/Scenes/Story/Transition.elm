...
import Canvas exposing (Point, Renderable, text, rect, shapes)
...
storyTransitionOut env rend f =
    let
        f1 = f*0.6
        str1 = "opacity(" ++ (String.fromInt (round ((1-f1)*100.0))) ++ "%)"
        str2 = "opacity(" ++ (String.fromInt (round (f1*100.0))) ++ "%)"
        rend1 = Canvas.group [ filter str1 ] [ rend ]
        rend2 = shapes  [ filter str2
...
hallTransitionIn env rend f =
    let
        ...
        str1 = "opacity(" ++ (String.fromInt (round ((1-f1)*100.0))) ++ "%)"
        str2 = "opacity(" ++ (String.fromInt (round (f1*100.0))) ++ "%)"
        rend1 = Canvas.group [ filter str2 ] [ rend ]
        rend2 = shapes  [ filter str1
                        , fill (Color.rgb255 20 30 40) ]
                        [ rect (posToReal env (0,0)) (lengthToReal env 1920) (lengthToReal env 1080) ]
    in
    Canvas.group
