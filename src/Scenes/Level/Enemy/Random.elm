...
curUniqueSin time loc id =
    let
        b =
            partialCoefficient * cellLength

        q =
            toFloat (Tuple.first loc) + toFloat (Tuple.second loc) + toFloat (id * 100 + time)

        c1 =
            sin (2 * degrees (toFloat (round q // period1)))

        c2 =
            sin (2 * degrees (toFloat (round q // period2)))

        c3 =
            sin (2 * degrees (toFloat (round q // period3)))
        ...
        x =
            List.map (listModifyNum (b / toFloat xNum)) (List.range 1 xNum)
    in
